...
function SignupCard({ error, isLoading, onGoogleSignIn }: SignupCardProps) {
  return (
    <motion.div
    ...
    >
      <h2 className="font-bold text-2xl sm:text-3xl text-white mb-2">Welcome to Kalendar</h2>
      <p className="text-purple-200 text-sm max-w-sm mb-6 sm:mb-8">
        Get started with your favourite Calendar app and optimize your daily routine with
        AI-powered insights
      </p>
      ...
      <div className="flex flex-col space-y-4">
        <Button
          ...
          startContent={!isLoading && <RiGoogleFill className="h-5 w-5" />}
        >
  ...
  return (
    <Layout>
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-b from-black via-purple-900 to-purple-400 py-12 sm:py-20 relative overflow-hidden px-4 sm:px-6 lg:px-8">
        <BackgroundCircle />
        <SignupCard error={error} isLoading={isLoading} onGoogleSignIn={signInWithGoogle} />
      </div>
    </Layout>
  );
}