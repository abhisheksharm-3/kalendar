...
const WeekView: React.FC<WeekViewProps> = ({
  ...
  const handleDrop = (e: React.DragEvent, dropDate: Date) => {
    ...
    // Parse the dragged event data
    const draggedEvent: Event = JSON.parse(e.dataTransfer.getData("text/plain"));
  
    // Get the position where the event was dropped
    ...
    // Calculate the new start date for the event
    ...
    // Calculate the new end date for the event
      ...
      <div className="flex items-center justify-center p-4 border-b dark:border-gray-700">
        <div className="text-center">
          <p className="text-xl lg:text-2xl text-gray-500 dark:text-gray-400">
            Week {getWeekNumber(date as Date)}
          </p>
        </div>
      </div>
      <div
                    ...
                    {events
                      ...
                      .map((event, eventIndex) => {
                        ...
                        return (
                          <div
                            key={event.id}
                            className={`absolute left-1 right-1 ${eventColor} text-white p-2 text-xs overflow-hidden rounded-lg shadow-md cursor-move ${
